...
The whole stack is defined in [`render.yaml`](render.yaml) as a Render Blueprint — API, Postgres,
Redis and the frontend, provisioned together.

### Deploy it

1. Push this repo to GitHub.
2. In Render: **New → Blueprint**, select the repo, click **Apply**.
3. Wait for the first build (~5 min; the Docker build compiles the jar).
4. Lock down CORS once the frontend URL exists — on **settleup-api → Environment**, set
   `CORS_ALLOWED_ORIGINS` to the exact static-site origin, e.g.
   `https://settleup-web.onrender.com`.

That last step is worth doing. The blueprint ships a `https://*.onrender.com` default because the
frontend's hostname isn't known until it deploys, and the two services can't reference each other
without creating a dependency cycle the Blueprint can't resolve.

### What gets created

| Resource | Type | Why |
|---|---|---|
| `settleup-api` | Docker web service | The Spring Boot API |
| `settleup-db` | Postgres | The ledger |
| `settleup-cache` | Key Value | Idempotency keys, balance cache, rate limits |
| `settleup-web` | Static site | Marketing page + app, on the CDN |

**The frontend is a static site, not served by the API.** Static sites are always-on and
edge-cached, so the landing page loads instantly even while a free-tier API instance is still
waking from cold start. Only the app behind sign-in waits.

### Configuration notes

- **`JWT_SECRET` is generated by Render**, never committed. Rotating it just invalidates
  existing tokens.
- **Postgres credentials are injected as parts**, not as a single URL. Render emits a libpq-style
  `postgres://` connection string, which the JDBC driver won't accept.
- **The API binds `$PORT`.** Render assigns it at runtime; a hard-coded 8080 would be unreachable.
- **`healthCheckPath` is `/api/health`**, which actually queries the database — an instance that
  can't reach Postgres never receives traffic.
- **The frontend's API host is baked in at build time** via `VITE_API_HOST`. Blueprints expose
  hostnames without a scheme and don't support interpolation, so the client prepends `https://`.
- **Free Postgres expires after 30 days.** Fine for a demo; upgrade or re-provision after that.

### Validating the blueprint before pushing
...
Checks `render.yaml` against Render's published JSON schema, so a bad enum or misplaced field
fails locally instead of halfway through a Blueprint sync.
...
**Ops** — Docker · GitHub Actions · Render
